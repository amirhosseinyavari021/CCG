...
export default function MarkdownBox({ markdown, lang = "fa" }) {
  const t = useMemo(() => {
    const fa = {
      copy: "کپی",
      copied: "کپی شد",
      ...
      <ReactMarkdown
        ...
        components={{
          code({ inline, className, children }) {
            ...
            if (!isBlock) {
              return <code className="ccg-inline-code">{children}</code>;
            }

            return (
              <div className="ccg-codeblock">
                <div className="ccg-codeblock-head">
                  <div className="ccg-codeblock-title">{(className || "").replace("language-", "") || "CODE"}</div>
