...
const Form = ({ onSubmit, onExplain, onScript, isLoading, loadingMessage, lang }) => {
    ...
    const validateAndSubmit = (handler) => {
        ...
        if (Object.keys(newErrors).length === 0) {
            handler({ os: finalOs, osVersion: finalOsVersion, cli: finalCli, userInput, deviceType: os === 'cisco' ? deviceType : undefined, knowledgeLevel });
        }
    };
        ...
        <div>
            ...
            <Card lang={lang}>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-4">
                    ...
                    {os !== 'other' ? (
                        ...
                        <>
                            ...
                            <CustomInput label={t.cli} value={customCli} onChange={setCustomCli} placeholder={t.customCliPlaceholder} error={formErrors.customCli} />
                        </>
                    )}
                </div>
                ...
                <div className="mt-5 grid grid-cols-1 md:grid-cols-3 gap-4">
                    <button onClick={() => validateAndSubmit(onSubmit)} disabled={isLoading} className="btn-primary flex items-center justify-center min-h-[48px]">
                        {isLoading ? <LoadingSpinner /> : <><Wand2 size={18} /><span className="ml-2">{t.generate}</span></>}
                    </button>
                    <button onClick={() => validateAndSubmit(onScript)} disabled={isLoading} className="w-full bg-emerald-700 text-white px-4 py-2.5 rounded-lg font-semibold hover:bg-emerald-800 disabled:bg-gray-400 flex items-center justify-center min-h-[48px]">
                        {isLoading ? <LoadingSpinner /> : <><FileCode2 size={18} /><span className="ml-2">{t.generateScript}</span></>}
                    </button>
                    <button onClick={() => validateAndSubmit(onExplain)} disabled={isLoading} className="w-full bg-sky-700 text-white px-4 py-2.5 rounded-lg font-semibold hover:bg-sky-800 disabled:bg-gray-400 flex items-center justify-center min-h-[48px]">
