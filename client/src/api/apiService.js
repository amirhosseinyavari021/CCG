...
ولید پرامپت سیستم متناسب با زبان و تنظیمات کاربر

export function getSystemPrompt(mode, os, osVersion, cli, lang, options) {
  if (lang === 'fa') {
    return `لطفاً خروجی و توضیحات را فقط به زبان فارسی تولید کن. سیستم عامل: ${os} نسخه: ${osVersion} ابزار: ${cli} نوع درخواست: ${mode}. اگر نیاز به دستور خاصی هست، با توضیح فارسی بنویس.`;
  } else {
    return `Please provide the output and explanations only in English. OS: ${os} version: ${osVersion} CLI: ${cli} Request type: ${mode}. If a specific command is needed, write it in English with English explanation.`;
  }
}
