...
import { Terminal, Copy, Check, ServerCrash, Wind, Apple, BrainCircuit, Bot, ChevronDown, Wand2, Search, LogIn, LogOut, User, History, Star, Sun, Moon, FileCode2, CopyPlus, Info, X, ShieldAlert, MessageSquare, Download } from 'lucide-react';
...
try {
  ...
  toast.error("Failed to connect to Firebase services. Some features may be unavailable.");
  db = null;
  auth = null;
}
...
const translations = {
  en: {
    ...
    copied: "Copied!", copyAll: "Copy All as Script", downloadScript: "Download Script", shareCommand: "Share Command", searchPlaceholder: "Search history or favorites...",
...
const Panel = ({ lang, onSelect, title, icon, collectionName, noItemsText }) => {
  ...
  useEffect(() => {
    ...
    const cacheKey = `cmdgen_${collectionName}_${user.uid}`;
  ...
  return (
    <motion.div className="w-full h-full bg-gray-100 dark:bg-gray-900 p-4 flex flex-col" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5 }}>
        ...
        <div className="flex-none flex items-center gap-4">
          ...
          {user ? (
            ...
            <div className="relative group">
              <button onClick={handleSignUpClick} className="flex items-center gap-2 bg-blue-600 text-white px-3 py-1.5 rounded-full text-sm font-semibold hover:bg-blue-700 transition-colors">
...
const Card = ({ children, lang }) => (
  <motion.div
    ...
    animate={{ opacity: 1, y: 0 }}
    transition={{ duration: 0.5 }}
    className="bg-white dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700 rounded-xl p-5 shadow-lg dark:shadow-2xl"
  ...
  const handlePrimaryAction = async () => {
    ...
    try {
      ...
      if (err.message.includes('API request failed')) {
        setError('Failed to connect to the command generation service. Please check your internet connection.');
      } else if (err.message.includes('empty response')) {
        setError('No commands were generated. Please try a different query.');
      } else {
        setError(t.errorMessage);
      }
      console.error('Error in fetch:', err);
      toast.error(error || t.errorMessage);
    } finally {
      setIsLoading(false);
    }
  };
    ...
    >
      ...
      <div className="flex-1 flex flex-col h-screen overflow-y-auto">
        ...
        <main className="flex-grow container mx-auto px-4 md:px-8 py-8 md:py-12">
          ...
          <motion.div
            className="mt-8 max-w-3xl mx-auto bg-white/50 dark:bg-gray-800/30 border border-gray-200 dark:border-gray-700/50 rounded-2xl p-6 shadow-lg backdrop-blur-md"
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.5 }}
          >
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="flex flex-col gap-2 md:col-span-3">
                <label className="text-sm font-medium text-gray-700 dark:text-gray-300">{t.os} <span className="text-red-500">*</span></label>
              ...
              <CustomSelect label={t.osVersion} value={osVersion} onChange={setOsVersion} options={osDetails[os].versions} placeholder={t.selectVersion} disabled={!os} lang={lang} error={errors.osVersion} />
              <CustomSelect label={t.cli} value={cli} onChange={setCli} options={osDetails[os].clis} placeholder={t.selectCli} disabled={!os} lang={lang} error={errors.cli} />
              <div className="flex flex-col gap-2 md:col-span-3">
                <label htmlFor="user-input" className="text-sm font-medium text-gray-700 dark:text-gray-300">{currentModeData.label} <span className="text-red-500">*</span></label>
