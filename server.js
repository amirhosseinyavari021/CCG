...
app.post('/api/proxy', async (req, res) => {
  ...
  try {
    ...
    const mode = systemMessage?.content?.includes('generate') ? 'generate' :
      systemMessage?.content?.includes('explain') ? 'explain' :
        systemMessage?.content?.includes('error') ? 'error' : 'unknown';
